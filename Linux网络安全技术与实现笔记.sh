			...
			7、CONNMARK的处理方法
				...
				用法举例：
					...
					iptables -t filter -A INPUT -m mark --mark 1 -j DROP
						上述例中，进入本机的数据包将会被丢弃掉，因为数据包进入本机后会先进入mangle表的INPUT链，然后才会进入filter的INPUT链，如此数据包
						在mangle表中会被标记nfmark为1，接着该数据包进入filter表之后，我们使用mark的匹配方式来匹配数据包，只要数据包上的nfmark为1，就会
						被丢弃掉
					iptables -t mangle -A INPUT -p tcp --dport 80 -j MARK --set-mark 1
					iptables -t filter -A OUTPUT -m mark --mark 1 -j DROP
						上述示例中将不会有任何数据包被丢弃，原因在于虽然进入本机的数据包在mangle表中被标记nfmark为1，但这个数据包一旦被本机的进程接收之后
						，就相当于数据包已经消失了，如此nfmark值也自然就跟着消失了，而本机应答给来源端主机的数据包则为另一个新生成的数据包，其上当然就不会
						有任何mark值存在。
				...
				用法举例：
					...
					以上两个示例中，数据包都被丢弃掉，因为CONNMARK的ctmark值有效范围为一完整的连接，因此不管是送入还是送出的数据包，其上都会包含这个cmark值
